...
int main() {
	...
	dimensioneFile = dimFile();
	printf("\n\nOra, la dimensione del file è: %d\n\n\n", dimensioneFile); // ora contando 1 byte per ogni char del file, abbiamo: 1 byte per ogni cifra, 1 byte per ogni spazio (uno dopo ogni cifra), un byte per ogni a capo. Il totale è quindi di 11 byte x 5 righe = 55 byte;  
